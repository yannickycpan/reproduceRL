...
from network.operations import compute_q_acted, update_target_nn_assign,\
    update_critic_ops, listshape2vec
from Agent import Agent
from sparseutils import get_grad_interferences
...
class DQN(FunctionApproximator):
    def __init__(self, params):
        ...
        self.statescale = params['statescale']
    ...
    def take_action(self, state):
        if len(state.shape) < 2 or (self.use_atari_nn and len(state.shape) < 4):
            state = state[None, :]
...
class DQNAgent(Agent):
    ...
    def custermized_log(self, logger):
        ...
        if self.replaybuffer.getSize() >= self.batchSize:
            ...
            print(' the overlap and instance are :: ==================== ', gradinfodict)
